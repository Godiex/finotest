...
public class ContentService
{
    ...
    {
        var segmentToDelete = await GetContentById(id);
        await _contentRepository.DeleteAsync(segmentToDelete);
    }
    private async Task<Content> GetContentById(Guid id)
    {
        var segmentToUpdate = await _contentRepository.GetByIdAsync(id);
        _ = segmentToUpdate ??
